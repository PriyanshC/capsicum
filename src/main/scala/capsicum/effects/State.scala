...
trait StateCapability[S, R] extends Capability[State[S], R, R] {
  final inline def get: (S => R) => R = perform(StateOp.Get())
...
class MutableStateHandler[S, R](private [effects] var state: S) extends StatefulCapability[S, R] with OneShotCapability[State[S], R, R] with NoMapResult[R] {
  override def handleEff[V](eff: StateEff[S, V]): V = eff match
