...
class MutableStateHandler[S, R](private [effects] var state: S) extends StatefulCapability[S, R] with OneShotKeepResult[State[S], R] {
...
object State {
  ...
  inline def runPureSafe[S, A, T[_]](inline initial: S)(inline prog: SafePureStateCapability[S, F] ?=> (S -> Bounce[(S, A)])): (S, A) = {
