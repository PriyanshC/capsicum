...
def basicMutableState(): Int = {
  val mutableHandler = new MutableStateHandler[Int](10)
  
  def prog(using state: StateCapability[Int]): Int = {
    val s1 = state.get()
    state.put(s1 + 5)
    val s2 = state.get()
    s2
...
//   val pureHandler = new PureStateCapability[Int]
  
//   def prog(using state: PureStateCapability[Int]): Int ->{state} (Int, Int) = {
//     state.get { s1 =>
//       state.put(s1 + 5) { _ =>
//         state.get { s2 =>
...
def trackedState(): Unit = {
  ...
  def progWithScopedCapture(using state: StateCapability[Logger^{fs}]): Unit = {
    val logger = state.get()
  ...
  def progWithPolymorphicCapture[C^](using state: StateCapability[Logger^{C}]): Unit = {
    val logger = state.get()
