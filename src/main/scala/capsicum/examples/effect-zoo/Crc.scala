...
object Crc {
  ...
  def consume1(stream: Flow[Chunked[Byte], Bounce]): Long =
    stream
      .filterChunks(_ < 128)
      .mapChunks(asciiToLower)
      .foldChunks(makeAccum())((acc, chunk) => acc ++ chunk.elements)
      .getValue()

  def consume2(stream: Flow[Byte, Bounce]): Long =
    stream
      .filter(_ < 128)
      .map(asciiToLower)
      .fold(makeAccum())(_ :+ _)
  ...
  def prechunked(elementss: IndexedSeq[Array[Byte]]): Flow[Chunked[Byte], Bounce] =
    Flow.fromPrechunkedSafe(elementss)

  def unchunked(elements: IndexedSeq[Byte]): Flow[Byte, Bounce] =
    Flow.fromSeqSafe(elements)
